                ...
                {loading ? (
                    ...
                    filteredTasks.map(task => (
                        <div
                            key={task._id}
                            onClick={() => navigate(`/admin/tasks/${task._id}`)}
                            className="glass-card p-4 space-y-3 active:scale-[0.98] transition-transform"
                        >
                            <div className="flex justify-between items-start gap-2">
                                <h3 className="font-bold text-white text-sm line-clamp-1">{task.title}</h3>
                                <span className={`shrink-0 px-2 py-0.5 rounded text-[10px] border ${getPriorityColor(task.priority)}`}>
                                    {task.priority}
                                </span>
                            </div>
                            <div className="flex items-center justify-between">
                                <div className="flex -space-x-1.5">
                                    {task.assignedTo?.map((u, i) => (
                                        <div key={i} className="w-6 h-6 rounded-full bg-indigo-600 border border-slate-900 flex items-center justify-center text-[10px] font-bold text-white">
                                            {u?.fullName?.[0]}
                                        </div>
                                    ))}
                                </div>
                                <div className={`text-[10px] font-semibold ${getStatusColor(task.status)}`}>
                                    {task.status}
                                </div>
                            </div>
                            <div className="flex items-center justify-between text-[10px] text-gray-500 pt-2 border-t border-white/5">
                                ...
                                <div className="flex items-center gap-2">
                                    <button onClick={(e) => handleWhatsAppShare(task, e)} className="p-1.5 text-green-400"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M7.9 20A9 9 0 1 0 4 16.1L2 22Z" /></svg></button>
                                    <button onClick={(e) => openEditModal(task, e)} className="p-1.5 text-blue-400"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg></button>
                                </div>
                            </div>
                        </div>
                    ))
