...
const PasswordManager = () => {
    ...
    useEffect(() => {
        if (user && user.role !== 'admin' && !user.permissions?.canManagePasswords) {
    ...
    const fetchCompanies = async () => {
        try {
            const res = await axios.get('/api/resources/companies'); // Employees should be able to see companies they are assigned to? Or all? Assuming all for now based on prompt simplicity
    ...
    const handleDelete = async (id) => {
        // Employees might not verify delete, but let's allow it for now or check backend perms. 
        // Prompt says "Admin Section ... as well the Employee side".
            ...
            <div className="mb-8 max-w-xl">
                ...
                <div className="relative">
                    ...
                    <select
                        ...
                        className="w-full bg-slate-800 border border-slate-700 rounded-lg py-2 pl-10 pr-4 text-white focus:outline-none focus:border-emerald-500"
                    >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    >
                        <h2 className="text-xl font-semibold mb-4 flex items-center gap-2">
                            <FaPlus className="text-emerald-400" /> Add New Passwords
                        </h2>
                            ...
                            {newCredentials.map((cred, index) => (
                                <div key={index} className="flex flex-col gap-2 p-3 bg-slate-900/50 rounded-lg border border-slate-700/50 relative">
                                    ...
                                    <input
                                        ...
                                        className="bg-slate-800 rounded px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500"
                                    />
                                    <div className="grid grid-cols-2 gap-2">
                                        <input
                                            ...
                                            className="bg-slate-800 rounded px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500" // Kept emerald focus ring for consistency or switch to color? Keeping emerald as base theme for employee page usually
                                        />
                                        <input
                                            type="text"
                                    ...
                                    <textarea
                                        ...
                                        className="bg-slate-800 rounded px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500 h-20 resize-none"
                                    />
                                    {/* Color Picker */}
                                    <div className="flex gap-2 mt-1">
                                        {Object.keys(colorMap).map(color => (
                                            <button
                                                key={color}
                                                onClick={() => handleInputChange(index, 'color', color)}
                                                className={`w-6 h-6 rounded-full border-2 transition-all ${cred.color === color ? 'border-white scale-110' : 'border-transparent opacity-50 hover:opacity-100'}`}
                        ...
                        <div className="flex gap-4 mt-6">
                            ...
                            <button
                                onClick={handleSave}
                                disabled={saving}
                                className="px-6 py-2 bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 rounded-lg text-sm font-semibold shadow-lg transition-all flex items-center gap-2 ml-auto"
                            >
                    ...
                    <div className="space-y-4">
                        <h2 className="text-xl font-semibold mb-4 flex items-center gap-2">
                            <FaLock className="text-emerald-400" /> Stored Credentials
                        </h2>
                            ...
                            <div className="space-y-3">
                                {credentials.map(cred => (
                                    <motion.div
                                        ...
                                        style={{ borderLeftColor: `var(--color-${cred.color || 'blue'}-500)` }}
                                    >
                                        <div className="relative overflow-hidden">
                                            <div className={`absolute top-0 left-0 w-1 h-full bg-${cred.color || 'blue'}-500`}></div>
                                            <div className="pl-3">
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm mb-3">
                                                    ...
                                                    <div className="bg-slate-900/50 p-2 rounded border border-slate-700/50 flex flex-col relative">
                                                        ...
                                                        <div className="flex items-center justify-between">
                                                            <span className={`font-mono ${cred.isRestricted ? 'text-red-400 italic' : 'text-gray-300'}`}>
                                                                {cred.isRestricted ? '[Restricted]' : (visiblePasswords[cred._id] ? cred.password : '••••••••••••')}
                                                            </span>
                                                            {!cred.isRestricted && (
                                                                <button
                                                ...
                                                {cred.isRestricted ? (
                                                    <div className="bg-red-900/20 p-2 rounded border border-red-500/20 text-xs text-red-400 italic mb-2">
                                                        Details are hidden due to insufficient permissions.
                                                    </div>
                                                ) : cred.details && (
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <input
                                ...
                                className="w-full bg-slate-800 rounded px-3 py-2 text-white focus:outline-none focus:ring-1 focus:ring-emerald-500"
                            />
                        </div>
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                className="w-full bg-slate-800 rounded px-3 py-2 text-white focus:outline-none focus:ring-1 focus:ring-emerald-500 h-24 resize-none"
                            />
                        </div>
                        <div>
                        ...
                        <div className="flex justify-end gap-3 mt-6">
                            ...
                            <button
                                ...
                                disabled={saving}
                                className="px-6 py-2 bg-gradient-to-r from-emerald-600 to-emerald-500 rounded-lg text-white font-semibold hover:from-emerald-500 hover:to-emerald-400 disabled:opacity-50"
                            >
